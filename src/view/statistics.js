...
import {countCompletedTaskInDateRange, makeItemsUnique, countTasksByColor, colorToHex, getDatesInRange, countTaskInDateRange, parseChartDate} from '../utils/statistics.js';

const renderColorChart = (colorsCtx, tasks) => {
  const taskColors = tasks.map((task) => task.color);
  const uniqColors = makeItemsUnique(taskColors);
...
export default class Statistics extends SmartView {
  constructor(tasks) {
    ...
    this._data = {
      tasks,
      dateFrom: (() => {
        const dayToFullWeek = 6;
        const date = getCurrentDate();
        date.setDate(date.getDate() - dayToFullWeek);
        return date;
      })(),
  ...
  _setCharts() {
    ...
    this._colorsCart = renderColorChart(colorsCtx, tasks);
